        ...
        female: Promise.resolve(_.rangeRight(parseInt(timeSpan.end, 10) || 0, (parseInt(timeSpan.start, 10) || 0) + 1)).map(function (item) {
            ...
            return Promise.reduce(data.male, function (accum, item, index) {
                if (timeSpan.start - timeSpan.end >= 12) {
                    ...
                    if (accum.dataset[0].data[resultIndex]) {
                        accum.dataset[0].data[resultIndex].value += verticalAxisTypeConverter.convert(item.avg, item.sum);
                        accum.dataset[1].data[resultIndex].value += verticalAxisTypeConverter.convert(data.female[index].avg, data.female[index].sum);
                    } else {
                        accum.dataset[0].data[resultIndex] = {
                            value: verticalAxisTypeConverter.convert(item.avg, item.sum)
                        };
                        accum.dataset[1].data[resultIndex] = {
                            value: verticalAxisTypeConverter.convert(data.female[index].avg, data.female[index].sum)
                        };
            ...
            numberSuffix: verticalAxisTypeConverter.suffix,    paletteColors: null
