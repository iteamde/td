...
function totalChartView(filterSql, accessLevelSql, verticalAxisTypeConverter, customField, customValue) {
    ...
    return Promise.resolve(_.rangeRight(parseInt(timeSpan.end, 10) || 0, (parseInt(timeSpan.start, 10) || 0) + 1)).map(function (item) {
        var query = 'SELECT ' +
            'AVG(`tbu`.`trendata_bigdata_user_absences`) AS `avg`, ' +
            'SUM(`tbu`.`trendata_bigdata_user_absences`) AS `sum`, ' +
        ...
        if (timeSpan.start - timeSpan.end >= 12) {
            ...
            if (accum.dataset[0].data[resultIndex]) {
                accum.dataset[0].data[resultIndex].value += verticalAxisTypeConverter.convert(item.avg, item.sum);
            } else {
                accum.dataset[0].data[resultIndex] = {
                    value: verticalAxisTypeConverter.convert(item.avg, item.sum)
                };
        ...
        nhp: Promise.resolve(_.rangeRight(parseInt(timeSpan.end, 10) || 0, (parseInt(timeSpan.start, 10) || 0) + 1)).map(function (item) {
            ...
            return Promise.reduce(data.hp, function (accum, item, index) {
                if (timeSpan.start - timeSpan.end >= 12) {
                    ...
                    if (accum.dataset[0].data[resultIndex]) {
                        accum.dataset[0].data[resultIndex].value += verticalAxisTypeConverter.convert(item.avg, item.sum);
                        accum.dataset[1].data[resultIndex].value += verticalAxisTypeConverter.convert(data.nhp[index].avg, data.nhp[index].sum);
                    } else {
                        accum.dataset[0].data[resultIndex] = {
                            value: verticalAxisTypeConverter.convert(item.avg, item.sum)
                        };
                        accum.dataset[1].data[resultIndex] = {
                            value: verticalAxisTypeConverter.convert(data.nhp[index].avg, data.nhp[index].sum)
                        };
                ...
                if (timeSpan.start - timeSpan.end >= 12) {
                    ...
                    if (accum.dataset[0].data[resultIndex]) {
                        _.each(item, function(value, subIndex) {
                            accum.dataset[subIndex].data[resultIndex].value += verticalAxisTypeConverter.convert(value.avg, value.sum);
                        });
                        } else {
                            _.each(item, function(value, subIndex) {
                                accum.dataset[subIndex].data[resultIndex] = {value: verticalAxisTypeConverter.convert(value.avg, value.sum)};
                        ...
                        _.each(item, function(value, subIndex) {
                            accum.dataset[subIndex].data.push({
                                value: verticalAxisTypeConverter.convert(value.avg, value.sum)
                            });
                        });
                    }
...
switch (requestData.type) {
    ...
    default:
        ...
        }).spread(function (accessLevelSql, filterSql, usersFilter, verticalAxisTypeConverter, availableFilters, customFields) {
            return Promise.props({
                /**
                 *
                 */
