    ...
    return Promise.map(values, function (item) {
        return Promise.props({
            total: orm.query(
                ...
                return  null === rows[0].total_count ? 0 : rows[0].total_count;
            }),
            yes: orm.query(
                'SELECT ' +
                'COUNT(*) AS `successors_count` ' +
                'FROM ' +
                '`trendata_bigdata_user` AS `tbu` ' +
                'WHERE ' +
                '((`tbu`.`trendata_bigdata_user_position_termination_date` IS NOT NULL AND `tbu`.`trendata_bigdata_user_position_hire_date` < DATE_FORMAT(NOW(), \'%Y-%m-01\') AND `tbu`.`trendata_bigdata_user_position_termination_date` >= DATE_FORMAT(NOW(), \'%Y-%m-01\')) ' +
                ...
                return  null === rows[0].successors_count ? 0 : rows[0].successors_count;
            })
        }).then(function(result) {
            return {
                label: item,
                yes: verticalAxisTypeConverter.convert(result.yes, result.total)
            };
        });
    }).reduce(function (accum, item) {
        if (requestData.data.hide_empty && ! item.yes)
            return accum;
