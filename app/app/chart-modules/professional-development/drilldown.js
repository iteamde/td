...
var calculateSubChartData = function(input) {
    ...
    return Promise.map(values, function (item) {
        return orm.query(
            'SELECT ' +
            'ROUND(AVG(`tbu`.`trendata_bigdata_user_performance_percentage_this_year`), 2) AS `average_prof_development`, ' +
            'SUM(`tbu`.`trendata_bigdata_user_performance_percentage_this_year`) AS `sum_prof_development` ' +
            'FROM ' +
            ...
            'AND ' +
            'UPPER(`tbu`.`trendata_bigdata_user_prof_development`) = \'YES\' ' +
            'AND ' +
            column + ' = ? ' +
            'AND ' +
            filterSql.query +
            ...
            return {
                label: item,
                value: verticalAxisTypeConverter.convert(rows[0].average_prof_development, rows[0].sum_prof_development)
            };
        });
