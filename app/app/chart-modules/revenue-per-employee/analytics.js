...
/**
 * Get gross revenue value
 * @return {Promise}
 */
function getGrossRevenueValue() {
    var FinancialDataModel = ormModels.FinancialData;
    var startDate = moment().subtract(timeSpan.start, 'month');
    var endDate = moment().subtract(timeSpan.end - 1, 'month');
    var startYear = +startDate.format('Y');
    var endYear = +endDate.format('Y');
    ...
    return FinancialDataModel.findAll({
        ...
        for (var i = startYear; i <= endYear; i++) {
            ...
            if (item = _.find(data, {trendata_financial_data_year: i})) {
                ...
                var grossRevenue = _.find(fromJson, {title: 'Gross Revenue'}).data;
                _.each(grossRevenue, function(monthRevenue) {
                    result.push(+monthRevenue.value);
                });
            } else {
        ...
        return Promise.resolve(_.slice(result, startDate.format('M') - 1, (endDate.format('M') - 12) || undefined));
    });
}
...
function totalChartView(filterSql, accessLevelSql, verticalAxisTypeConverter, customField, customValue) {
    ...
    return getGrossRevenueValue().then(function (grossRevenueValues) {
            ...
            if (customField) {
                ...
                if (timeSpan.start - timeSpan.end >= 12) {
                    ...
                    if (accum.dataset[0].data[resultIndex]) {
                        accum.dataset[0].data[resultIndex].value += verticalAxisTypeConverter.convert(grossRevenueValues[index] / item.count, grossRevenueValues[index]);
                    } else {
                        accum.dataset[0].data[resultIndex] = {
                            value: verticalAxisTypeConverter.convert(grossRevenueValues[index] / item.count, grossRevenueValues[index])
                        };
            ...
            female: Promise.resolve(_.rangeRight(parseInt(timeSpan.end, 10) || 0, (parseInt(timeSpan.start, 10) || 0) + 1)).map(function (item) {
                ...
                return Promise.reduce(data.male, function (accum, item, index) {
                    if (timeSpan.start - timeSpan.end >= 12) {
                        ...
                        if (accum.dataset[0].data[resultIndex]) {
                            accum.dataset[0].data[resultIndex].value += verticalAxisTypeConverter.convert(grossRevenueValues[index] / item.count, grossRevenueValues[index]);
                            accum.dataset[1].data[resultIndex].value += verticalAxisTypeConverter.convert(grossRevenueValues[index] / data.female[index].count, grossRevenueValues[index]);
                        } else {
                            accum.dataset[0].data[resultIndex] = {
                                value: verticalAxisTypeConverter.convert(grossRevenueValues[index] / item.count, grossRevenueValues[index])
                            };
                            accum.dataset[1].data[resultIndex] = {
                                value: verticalAxisTypeConverter.convert(grossRevenueValues[index] / data.female[index].count, grossRevenueValues[index])
                            };
        ...
        return Promise.props({
            ...
            return Promise.reduce(data.hp, function (accum, item, index) {
                if (timeSpan.start - timeSpan.end >= 12) {
                    ...
                    if (accum.dataset[0].data[resultIndex]) {
                            accum.dataset[0].data[resultIndex].value += verticalAxisTypeConverter.convert(grossRevenueValues[index] / item.count, grossRevenueValues[index]);
                            accum.dataset[1].data[resultIndex].value += verticalAxisTypeConverter.convert(grossRevenueValues[index] / data.nhp[index].count, grossRevenueValues[index]);
                    } else {
                        accum.dataset[0].data[resultIndex] = {
                            value: verticalAxisTypeConverter.convert(grossRevenueValues[index] / item.count, grossRevenueValues[index])
                        };
                        accum.dataset[1].data[resultIndex] = {
                            value: verticalAxisTypeConverter.convert(grossRevenueValues[index] / data.nhp[index].count, grossRevenueValues[index])
                        };
                    ...
                    if (timeSpan.start - timeSpan.end >= 12) {
                        ...
                        if (accum.dataset[0].data[resultIndex]) {
                            _.each(item, function(value, subIndex) {
                                accum.dataset[subIndex].data[resultIndex].value += verticalAxisTypeConverter.convert(grossRevenueValues[index] / value.count, grossRevenueValues[index]);
                            });
                        } else {
                            _.each(item, function(value, subIndex) {
                                accum.dataset[subIndex].data[resultIndex] = {value: verticalAxisTypeConverter.convert(grossRevenueValues[index] / value.count, grossRevenueValues[index])};
                        ...
                        _.each(item, function(value, subIndex) {
                            accum.dataset[subIndex].data.push({
                                value: verticalAxisTypeConverter.convert(grossRevenueValues[index] / value.count, grossRevenueValues[index])
                            });
                        });
                    }
        ...
        return Promise.props({
            ...
            chart_data: (function () {
                ...
                if (requestData.data.regression_analysis) {
                    ...
                    }).map(function (item) {data.debug.output.push(item);
                        return {
                            color: '#008ee4',
                            dashed: '0',
