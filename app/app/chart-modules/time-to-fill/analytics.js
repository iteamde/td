...
function totalChartView(filterSql, accessLevelSql, verticalAxisTypeConverter, customField, customValue) {
    ...
    return Promise.resolve(_.rangeRight(parseInt(timeSpan.end, 10) || 0, (parseInt(timeSpan.start, 10) || 0) + 1)).map(function (item) {
        var query = 'SELECT ' +
            'AVG(DATEDIFF(`tbup`.`trendata_bigdata_user_position_hire_date`, `tbu`.`trendata_bigdata_user_posting_date`)) AS `avg`, ' +
            'SUM(DATEDIFF(`tbup`.`trendata_bigdata_user_position_hire_date`, `tbu`.`trendata_bigdata_user_posting_date`)) AS `avg_sum`, ' +
            ...
            'AND ' +
            '`tbu`.`trendata_bigdata_user_salary` IS NOT NULL ' +
            'AND ' +
            filterSql.query +
            ' AND ' +
