...
var calculateSubChartData = function(input) {
    ...
    return Promise.map(values, function (item) {
        return orm.query(
            'SELECT ' +
            'ROUND(AVG(DATEDIFF(`tbu`.`trendata_bigdata_user_position_hire_date`, `tbu`.`trendata_bigdata_user_posting_date`)), 2) AS `average_time_to_fill`, ' +
            'SUM(DATEDIFF(`tbu`.`trendata_bigdata_user_position_hire_date`, `tbu`.`trendata_bigdata_user_posting_date`)) AS `sum_time_to_fill` ' +
            'FROM ' +
            '`trendata_bigdata_user` AS `tbu` ' +
            'WHERE ' +
            '`tbu`.`trendata_bigdata_user_position_hire_date` IS NOT NULL ' +
            'AND ' +
            '(`tbu`.`trendata_bigdata_user_position_hire_date` >= DATE_FORMAT(NOW() - INTERVAL 1 MONTH, \'%Y-%m-01\') AND `tbu`.`trendata_bigdata_user_position_hire_date` < DATE_FORMAT(NOW(), \'%Y-%m-01\')) ' +
            'AND ' +
            column + ' = ? ' +
            'AND ' +
            filterSql.query +
