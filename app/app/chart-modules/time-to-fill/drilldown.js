...
var calculateSubChartData = function(input) {
    ...
    return Promise.map(values, function (item) {
        return orm.query(
            'SELECT ' +
            'ROUND(AVG(DATEDIFF(`tbup`.`trendata_bigdata_user_position_hire_date`, `tbu`.`trendata_bigdata_user_posting_date`)), 2) AS `average_time_to_fill`, ' +
            'SUM(DATEDIFF(`tbup`.`trendata_bigdata_user_position_hire_date`, `tbu`.`trendata_bigdata_user_posting_date`)) AS `sum_time_to_fill` ' +
            'FROM ' +
            ...
            return {
                label: item,
                value: verticalAxisTypeConverter.convert(rows[0].average_time_to_fill, rows[0].sum_time_to_fill)
            };
        });
