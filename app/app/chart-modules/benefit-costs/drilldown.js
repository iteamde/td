 ...
 * @param offset
 */
function getCostOfBenefit(offset) {
    var FinancialDataModel = ormModels.FinancialData;
    var currentDate = new Date();
    var lastMonth = new Date(currentDate.getFullYear(), currentDate.getMonth() + offset, 3);
    ...
    return FinancialDataModel.findOne({
        ...
        return parseFloat(value && value[0] && value[0].data && value[0].data[valueMonth] && value[4].data[valueMonth].value || 0);
    });
}
...
var calculateSubChartData = function(input) {
    ...
    return Promise.map(values, function (item) {
        return orm.query(
            'SELECT ' +
            'ROUND(AVG(`tbu`.`trendata_bigdata_user_benefit_costs`), 2) AS `average_benefits_cost`, ' +
            'SUM(`tbu`.`trendata_bigdata_user_benefit_costs`) AS `sum_benefits_cost` ' +
            'FROM ' +
            ...
            return Promise.props({
                label: item,
                // value: null === rows[0].average_benefits_cost ? getCostOfBenefit(-1) : rows[0].average_benefits_cost
                value: (function () {
                    if (null === rows[0].average_benefits_cost) {
                        return getCostOfBenefit(-1).then(function (value) {
                            return verticalAxisTypeConverter.convert(value, rows[0].sum_benefits_cost);
                        });
                    }

                    return verticalAxisTypeConverter.convert(rows[0].average_benefits_cost, rows[0].sum_benefits_cost);
                })()
            });
        });
