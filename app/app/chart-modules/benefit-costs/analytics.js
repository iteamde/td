...
function totalChartView(filterSql, accessLevelSql, verticalAxisTypeConverter, customField, customValue) {
    ...
    return Promise.resolve(_.rangeRight(parseInt(timeSpan.end, 10) || 0, (parseInt(timeSpan.start, 10) || 0) + 1)).map(function (item) {
        ...
        switch (yearDiff) {
            case 4:
                var benefitCostsColumn = '`tbu`.`trendata_bigdata_user_benefit_costs_4_year_ago`';
                break;
            case 3:
                var benefitCostsColumn = '`tbu`.`trendata_bigdata_user_benefit_costs_3_year_ago`';
                break;
            case 2:
                var benefitCostsColumn = '`tbu`.`trendata_bigdata_user_benefit_costs_2_year_ago`';
                break;
            case 1:
                var benefitCostsColumn = '`tbu`.`trendata_bigdata_user_benefit_costs_1_year_ago`';
                break;
            case 0:
            default:
                var benefitCostsColumn = '`tbu`.`trendata_bigdata_user_benefit_costs`';
        }

        var query = 'SELECT ' +
            'AVG(' + benefitCostsColumn + ') AS `avg`, ' +
            'SUM(' + benefitCostsColumn + ') AS `avg_sum`, ' +
            ...
            'AND ' +
            benefitCostsColumn + ' IS NOT NULL ' +
            'AND ' +
            filterSql.query +
            ' AND ' +
