...
function totalChartView(filterSql, accessLevelSql, verticalAxisTypeConverter, customField, customValue) {
    ...
    return Promise.resolve(_.rangeRight(parseInt(timeSpan.end, 10) || 0, (parseInt(timeSpan.start, 10) || 0) + 1)).map(function (item) {
        var query = 'SELECT ' +
            'IF(`tbu`.`trendata_bigdata_user_rehire_date` != NULL, DATEDIFF(DATE_FORMAT(NOW() + INTERVAL (1 + ?) MONTH, \'%Y-%m-01\'), `tbu`.`trendata_bigdata_user_rehire_date`), DATEDIFF(DATE_FORMAT(NOW() + INTERVAL (1 + ?) MONTH, \'%Y-%m-01\'), `tbu`.`trendata_bigdata_user_position_hire_date`)) AS `tenure` ' +
            'FROM ' +
            '`trendata_bigdata_user` AS `tbu` ' +
            'WHERE ' +
