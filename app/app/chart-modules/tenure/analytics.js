...
function totalChartView(filterSql, accessLevelSql, verticalAxisTypeConverter, customField, customValue) {
    ...
    return Promise.resolve(_.rangeRight(parseInt(timeSpan.end, 10) || 0, (parseInt(timeSpan.start, 10) || 0) + 1)).map(function (item) {
        var query = 'SELECT ' +
            'IF(`tbu`.`trendata_bigdata_user_rehire_date` != NULL, DATEDIFF(DATE_FORMAT(NOW() + INTERVAL (1 + ?) MONTH, \'%Y-%m-01\'), `tbu`.`trendata_bigdata_user_rehire_date`), DATEDIFF(DATE_FORMAT(NOW() + INTERVAL (1 + ?) MONTH, \'%Y-%m-01\'), `tbup`.`trendata_bigdata_user_position_hire_date`)) AS `tenure` ' +
            'FROM ' +
            '`trendata_bigdata_user` AS `tbu` ' +
            'INNER JOIN ' +
        ...
        query = 'SELECT ' +
            ...
            return orm.query(query, {
                ...
                return {
                    ...
                    avg: _.round(rows[0].avg / 365.25, 2),
                    avg_sum: rows[0].avg_sum / 365.25
                };
            });
        ...
        male: Promise.resolve(_.rangeRight(parseInt(timeSpan.end, 10) || 0, (parseInt(timeSpan.start, 10) || 0) + 1)).map(function (item) {
            ...
            return orm.query(query, {
                ...
                return {
                    month_name: moment(rows[0].month).format('MMMM'),
                    avg: _.round(rows[0].avg / 365.25, 2),
                    avg_sum: _.round(rows[0].avg_sum / 365.25, 2)
                };
            });
        }),
