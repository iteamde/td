...
var calculateSubChartData = function (input) {
    ...
    return Promise.map(values, function (item) {
        return orm.query(
            ...
            'SUM(`tbu`.`trendata_bigdata_user_salary`) AS `salary_sum`, ' +
            'SUM(`tbu`.`trendata_bigdata_user_industry_salary`) AS `industry_salary_sum` ' +
            'FROM ' +
            ...
            return {
                label: item,
                average_salary: verticalAxisTypeConverter.convert(rows[0].average_salary, rows[0].salary_sum),
                industry_salary: verticalAxisTypeConverter.convert(rows[0].industry_salary, rows[0].industry_salary_sum)
            };
        });
