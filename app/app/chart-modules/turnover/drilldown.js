    ...
    return Promise.map(values, function (item) {
        return Promise.props({
            ...
            terminated: orm.query(
                ...
                'AND ' +
                'DATE_FORMAT(`tbup`.`trendata_bigdata_user_position_termination_date`, \'%Y-%m\') = DATE_FORMAT(NOW() - INTERVAL 1 MONTH, \'%Y-%m\') ' +
                'AND ' +
                column + ' = ? ' +
                'AND ' +
                filterSql.query +
...
if ('change_filters' === requestData.type) {
    Promise.all([
        commonChartData.getAvailableFiltersForDrilldown(req).then(function (availableFilters) {
            return orm.query(
                'SELECT ' +
