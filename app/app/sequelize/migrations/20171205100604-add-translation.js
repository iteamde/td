...
module.exports = {
  up: function (queryInterface, Sequelize) {
    return Promise.all([
        addTranslation(queryInterface, Sequelize, 'email_addresses', 'Email Address(es)', 1),
        addTranslation(queryInterface, Sequelize, 'customized_comment', 'Customized Comment (Optional)', 1)
    ]);
  },

  down: function (queryInterface, Sequelize) {
    return Promise.all([
        removeTranslation(queryInterface, 'email_addresses', 1),
        removeTranslation(queryInterface, 'customized_comment', 1)
    ]);
  }
};
